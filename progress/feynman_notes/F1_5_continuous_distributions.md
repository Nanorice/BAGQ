...
4. Binomial is sequence of events, each with a binary results so a Bernoulli, modelling for total number of successes given n and p; Poission is binomial for large n and small p, with moderte np; exponential is for measuring continuous of variable passage until the first occurance of a event; 
...
1. didn't know about inverse transformation + mechanism in inverse transform to get distribution:
	1. inverse transformaion: samples a distribution from the one we have. We feed U~uniform(0,1) into known F^-1 to get draws of X.
2. E(x) and Var(x) for exponential should be 1/k, 1/k^2
...
3. geometric adds to give negative binomial; exponential adds to give Gamma. 
...
2. exponential measure arrival rate and constant hazard rate, lambda (= (d F(t) /dt)/(1-F(t))) , used for order filling in market making
...
| ---------------- | ----------------------------- | ----------------------------------------- | ------- | ---------- | --- | ----------------------------- |
| **Uniform(a,b)** | 1/(b-a)                       | (x-a)/(b-a)                               | (b+a)/2 | (b-a)^2/12 | x   | distribution same as inversed |
| **Exp(λ)**       | k*exp(-kx)                    | 1-exp(-kx)                                | 1/k     | 1/k^2      | x   | mean = SD                     |
| **N(μ,σ²)**      | 1/sqrt(2pi)/sigma*exp(-z^2/2) | 1/sqrt(2pi)/sigma*integral(exp(-z^2/2)dz) | mu      | sigma^2    | x   | can be standardised           |

**Numerical anchors (from memory):** `P(|Z|>2) = __0.0455__` · one-tail 95% z = 1.645____ · two-tail 95% z = 1.96____

**Poisson ↔ Exponential:** <!-- one line -->
Poisson is measuring how many events per given time window. Exponential is measuring time before event occurs, which is continuous. 
## 6. Where this breaks
...
memorylessness is wrong for wear-out (physical parts) — hazard rises
normal approx to binomial needs large n AND p not near 0/1
